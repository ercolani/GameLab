    ...
    void DrawDynamicInspector(Material material, MaterialEditor materialEditor, MaterialProperty[] props)
    {
        ...
        else
        {
            for (int i = 0; i < props.Length; i++)
            {
                ...
                if (material.HasProperty("_LeavesColorMode"))
                {
                    if (material.GetFloat("_LeavesColorMode") == 0 && prop.name == "_LeavesColorFilter")
                        continue;

                    if (material.GetFloat("_LeavesColorMode") == 0 && prop.name == "_LeavesColorRange")
                        continue;
                }
        ...
        for (int i = 0; i < customPropsList.Count; i++)
        {
            ...
            {
                if (displayName.Contains("Color Mask") && shaderName.Contains("Grass") && !shaderName.Contains("Simple Lit"))
                {
                    displayName = "Color Mask (Bottom Mask)";
                }

                if (displayName.Contains("Color Mask") && shaderName.Contains("Leaf") && !shaderName.Contains("Simple Lit"))
                {
                    displayName = "Color Mask (Mask Blue)";
                }

                if (displayName.Contains("Color Mask") && shaderName.Contains("Cross") && !shaderName.Contains("Simple Lit"))
                {
                    displayName = "Color Mask (Mask Blue)";
                }
                ...
                {
                    displayName = "Alpha Mask (Mask Blue)";
                }
